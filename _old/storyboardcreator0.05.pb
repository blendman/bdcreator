...
; - Case : Pouvoir ajouter un personnage (issu du dossier par defaut). 
; - Bank Canvas (folder characters -> clique et ça le place). 
...
; - Window_EditCase() : add panelgdget (+ tab : bank,graphics)
...
Procedure Window_EditCase_SetGadgetState()
  ...
  EndIf
EndProcedure
Procedure Case_AddObject(typ=0)
  
  If typ=1
    ; text
    text$ = InputRequester(lang("text"), lang("Add a text"),"")
    If text$ <> #Empty$
      s.d = 2500/project\Width
      ...
      If StartVectorDrawing(ImageVectorOutput(img))
        ...
        VectorFont(FontID(0), 50 * s)
        VectorSourceColor(RGBA(0,0,0,255))
        MovePathCursor(0,h1*0.16)
        DrawVectorParagraph(text$,w2,h,#PB_VectorParagraph_Center)
        StopVectorDrawing()
      EndIf
    EndIf
  ...
  If OpenWindow(#BDC_Win_EditCase, 0, 0, winW, winH, Lang("Edit case"), #PB_Window_SystemMenu | #PB_Window_ScreenCentered|#PB_Window_SizeGadget, WindowID(#BDC_Win_Main))
    ...
    If ScrollAreaGadget(#PB_Any,x,y,wp,h0,wp-25,h0+200)
      ...
      If cont>0
        ...
        AddGadgetItem(#G_win_EditCase_ObjetType,0,lang("Image"))
        AddGadgetItem(#G_win_EditCase_ObjetType,1,lang("Text"))
        SetGadgetState(#G_win_EditCase_ObjetType,0)
...
If OpenWindow(#BDC_Win_Main, 0, 0, winW, winH, "BD creator (Storyboard & comics creation) "+#BDC_ProgramVersion, #PB_Window_SystemMenu | #PB_Window_ScreenCentered|#PB_Window_SizeGadget|#PB_Window_MaximizeGadget|#PB_Window_MinimizeGadget|#PB_Window_Maximize)
  ...
  ;Debug font$
  ;If RegisterFontFile(font$)
    ;LoadFont(0, "VTCLettererProRegular", 50)
  ;Else
...
; CursorPosition = 1362
; FirstLine = 315
; Folding = RhBgEAAPAAAAAAAAAAIAd8DAAsNGAAAAAQAIAIsI9B1IAAAAg
; EnableXP
; Warnings = Display
; EnablePurifier