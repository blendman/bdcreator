...
Procedure Createproject(name$="New project",w=2500,h=3500,spacelineH=50,spaceCaseW=50,nbpage=1,margeL=100,margeR=100,MargeTop=100,margeBottom=100)
  ...
  With Project
    ...
    \page(0)\MargeBottom = margeBottom
    \page(0)\MargeTop = MargeTop
    \page(0)\MargeRight = margeR
    \page(0)\MargeLeft = margeL
    \page(0)\SpacebetweenCase = spaceCaseW
    \page(0)\SpacebetweenLine = spacelineH    
...
If ExamineDesktops()
  ...
  winW = DesktopWidth(0)
EndIf
If OpenWindow(#VD_Win_Storyboard, 0, 0, winW, winH, "BD creator (Storyboard & comics creation)", #PB_Window_SystemMenu | #PB_Window_ScreenCentered|#PB_Window_SizeGadget|#PB_Window_MaximizeGadget|#PB_Window_MinimizeGadget|#PB_Window_Maximize)
  
  WindowBounds(#VD_Win_Storyboard,600,600,5000,5000)
  
  VDOptions\ModeAutomatic = 1
  VDOptions\Zoom = 17
  ; statusbar
  CreateStatusBar(#VD_StatusbarMain,WindowID(#VD_Win_Storyboard))
  ...
  ; menu
  If CreateMenu(#BDC_Menu_Main, WindowID(#VD_Win_Storyboard))
    MenuTitle(Lang("File"))
...
;     MenuItem(#BDC_Menu_ExportPageTemplate, lang("Export page as template"))
  ...
  ; add gadgets
  x = 5 : y=5 : wp = 230 : hp= winH-75-StatusBarHeight(#VD_StatusbarMain)-MenuHeight() : w1 = wp-(2*wp)/5-30 : h1 = 30 : a=2
  VDOptions\panelW = wp
